...
SUPPA=$(grep 'SUPPA' myPaths.path | cut -d "=" -f2)
FSPATH=$(grep 'FSPATH' myPaths.path | cut -d "=" -f2)
...
if [ "$eventTypeflag" == "" ]
	...
	then
		...
		python3.4 $SUPPA/suppa.py generateEvents -i $GTF_ANN -o $INT/2-SuppaEvents/ -e SE SS RI MX FL
...
export PATH=$PATH:$FSPATH/bin
cp $FSPARA $FSPATH/fluxPara.par

if [ "$eventTypeflag" == "" ]
	then
		python $FUNCTIONS/generate_rnaseq.py $INT/4-CalNBCounts/group1.nbcounts $MASTERDIR/DiffASgenes.txt $FSPATH/fluxPara.par $INT/5-$fgflag/$fgflag -p $fgpalt -c $meanbasecoverage -r $replflag
		python $FUNCTIONS/generate_rnaseq.py $INT/4-CalNBCounts/group2.nbcounts $MASTERDIR/DiffASgenes.txt $FSPATH/fluxPara.par $INT/6-$sgflag/$sgflag -p $sgpalt -c $meanbasecoverage -r $replflag
	...
	if [ -d "$INT/5-$fgflag" ]; 
 	...
 	mkdir "$INT/5-$fgflag"
	python $FUNCTIONS/generate_rnaseq.py $INT/4-CalNBCounts/group1.nbcounts $INT/4a-DiffASgenesIDs.txt $FSPATH/fluxPara.par $INT/5-$fgflag/$fgflag -p $fgpalt -c $meanbasecoverage -r $replflag
	if [ -d "$INT/6-$sgflag" ]; 
		then 
   				rm -r "$INT/6-$sgflag" 
 	fi
 	mkdir "$INT/6-$sgflag"
	python $FUNCTIONS/generate_rnaseq.py $INT/4-CalNBCounts/group2.nbcounts $INT/4a-DiffASgenesIDs.txt $FSPATH/fluxPara.par $INT/6-$sgflag/$sgflag -p $sgpalt -c $meanbasecoverage -r $replflag
	...
	then	
		...
		then 
 		...
 		for i in `seq 1 3`; do
     		...
     		gsnap -d 7-Gmap_genome -D $INT -s 9-splicesites_map.iit $file1 $file2 -m 10 --format=sam > "$SIMBAM/$fgflag"_"$i".sam
     		echo 'Converting sam to bam.'
     		samtools sort "$SIMBAM/$fgflag"_"$i".sam | samtools view -bS > "$SIMBAM/$fgflag"_"$i".bam
     		rm "$SIMBAM/$fgflag"_"$i".sam
     		gsnap -d $INT/7-Gmap_genome -D $INT -s 9-splicesites_map $file3 $file4 -m 10 --format=sam > "$SIMBAM/$sgflag"_"$i".sam
     		samtools sort "$SIMBAM/$sgflag"_"$i".sam | samtools view -bS  > "$SIMBAM/$sgflag"_"$i".bam
     		rm "$SIMBAM/$sgflag"_"$i".sam
     		echo 'Converting sam to bam.'
 		done
fi
